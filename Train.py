...
def val(model, epoch, save_path, writer):
    ...
    with torch.no_grad():
        ...
        test_loader = test_dataset(image_root=opt.test_path + '/rgb/',
                            gt_root=opt.test_path + '/gt/',
                            testsize=opt.trainsize)
...
def train(train_loader, model, optimizer, epoch, test_path):
    ...
    if epoch % opt.epoch_save == 0:
        if epoch % 3 == 1:
            torch.save(model.state_dict(), save_path  + 'PlantCAMO1.pth')
        elif epoch % 3 ==2 :
            torch.save(model.state_dict(), save_path  + 'PlantCAMO2.pth')
        elif epoch % 3 ==0 :
            torch.save(model.state_dict(), save_path  + 'PlantCAMO3.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str,default='./datasets/PlantCAMO1250/train',help='path to train dataset')
    parser.add_argument('--test_path', type=str,default='./datasets/PlantCAMO1250/test',help='path to testing dataset')
